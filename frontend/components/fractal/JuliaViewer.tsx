...
export default function JuliaViewer({ rustServiceUrl }: JuliaViewerProps) {
  ...
  const [juliaCImag, setJuliaCImag] = useState(0.27)
  ...
  const generateFractal = async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        julia_c_real: juliaCReal.toString(),
        julia_c_imag: juliaCImag.toString(),
      })
  ...
  const handlePresetSelect = (preset: any) => {
    ...
    if (preset.params.juliaCReal !== undefined) setJuliaCReal(preset.params.juliaCReal)
    if (preset.params.juliaCImag !== undefined) setJuliaCImag(preset.params.juliaCImag)
  }
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">
            Beautiful fractal sets with varying complex parameters
          </p>
        </div>
      </div>
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">
                  C Real: {juliaCReal.toFixed(3)}
                </label>
                <input
                  type="range"
                  value={juliaCReal}
                  onChange={(e) => setJuliaCReal(parseFloat(e.target.value))}
                  min={-2}
                  max={2}
                  step={0.01}
                  disabled={loading}
              ...
              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">
                  C Imaginary: {juliaCImag.toFixed(3)}
                </label>
                <input
                  type="range"
                  value={juliaCImag}
                  onChange={(e) => setJuliaCImag(parseFloat(e.target.value))}
                  min={-2}
                  max={2}
                  step={0.01}
                  disabled={loading}
